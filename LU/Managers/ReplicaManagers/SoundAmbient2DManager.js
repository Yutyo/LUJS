...
class SoundAmbient2DManager extends GenericManager {
    constructor(server) {
        ...
        server.eventBus.on('new-object-created', object => {
            if(object.components.hasComponent(Components.SOUND_AMBIENT_2D_COMPONENT)) {
                // This component doesn't have a serialization, so we add a blank one.
                object.addSerializer(SerializationOrder.indexOf(Components.SOUND_AMBIENT_2D_COMPONENT), (type, stream) => {});
            }
        });
    }
}
